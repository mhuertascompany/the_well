...
Input format (one file per simulation pair):
    Each HDF5 file contains two datasets:
      "raw"   — shape (2, 64, 64, 64): input  fields [density, energy] at t0
      "label" — shape (2, 64, 64, 64): output fields [density, energy] at t0 + 20 Myr
...
def write_well_hdf5(output_path: str, sim_files: list[str]) -> None:
    ...
    for i, sim_path in enumerate(sim_files):
        with h5py.File(sim_path, "r") as src:
            raw   = src["raw"][:]    # (2, 64, 64, 64)
            label = src["label"][:]  # (2, 64, 64, 64)

        # axis-0 of raw/label: [density=0, energy=1]
        density[i, 0] = raw[0]    # density  at t0
        density[i, 1] = label[0]  # density  at t0 + 20 Myr
        energy[i, 0]  = raw[1]    # energy   at t0
        energy[i, 1]  = label[1]  # energy   at t0 + 20 Myr
